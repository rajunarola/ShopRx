...
function BindDealRequest(tableId, status) {
  ...
  dynamic = $(table.id).DataTable({
    "sAjaxSource": "/Distributor/ManageAdvertisement/GetProductPriceIncreaseList?status=" + status,
    ...
    "rowCallback": function (row, data, index) {
      ...
      if (status !== 2) {
          ...
          // Changing row Color When priceIncreateDate is expired      
          var currentDate = new Date(Date());
          if (Date.parse(data.priceIncreateDate) < Date.parse(currentDate)) {
              $('td', row).css({ 'background-color': '#ff3333', 'color': 'white' });
          }
