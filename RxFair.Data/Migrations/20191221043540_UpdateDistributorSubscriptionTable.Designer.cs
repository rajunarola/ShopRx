// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using RxFair.Data.DbContext;

namespace RxFair.Data.Migrations
{
    [DbContext(typeof(RxFairDbContext))]
    [Migration("20191221043540_UpdateDistributorSubscriptionTable")]
    partial class UpdateDistributorSubscriptionTable
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "2.2.4-servicing-10062")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole<long>", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken();

                    b.Property<string>("Discriminator")
                        .IsRequired();

                    b.Property<string>("Name")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles");

                    b.HasDiscriminator<string>("Discriminator").HasValue("IdentityRole<long>");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<long>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType");

                    b.Property<string>("ClaimValue");

                    b.Property<long>("RoleId");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<long>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType")
                        .HasMaxLength(50);

                    b.Property<string>("ClaimValue")
                        .HasMaxLength(200);

                    b.Property<string>("Discriminator")
                        .IsRequired();

                    b.Property<long>("UserId");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");

                    b.HasDiscriminator<string>("Discriminator").HasValue("IdentityUserClaim<long>");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<long>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasMaxLength(128);

                    b.Property<string>("ProviderKey")
                        .HasMaxLength(128);

                    b.Property<string>("ProviderDisplayName");

                    b.Property<long>("UserId");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<long>", b =>
                {
                    b.Property<long>("UserId");

                    b.Property<long>("RoleId");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<long>", b =>
                {
                    b.Property<long>("UserId");

                    b.Property<string>("LoginProvider")
                        .HasMaxLength(128);

                    b.Property<string>("Name")
                        .HasMaxLength(128);

                    b.Property<string>("Value");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("RxFair.Data.DbContext.ApplicationUser", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("AccessFailedCount");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken();

                    b.Property<long?>("DistributorId");

                    b.Property<string>("Email")
                        .HasMaxLength(100);

                    b.Property<bool>("EmailConfirmed");

                    b.Property<string>("FaxNumber")
                        .HasMaxLength(20);

                    b.Property<string>("FirstName")
                        .HasMaxLength(50);

                    b.Property<bool>("IsActive");

                    b.Property<string>("JobTitle")
                        .HasMaxLength(30);

                    b.Property<DateTime?>("LastLogin");

                    b.Property<string>("LastName")
                        .HasMaxLength(50);

                    b.Property<bool>("LockoutEnabled");

                    b.Property<DateTimeOffset?>("LockoutEnd");

                    b.Property<string>("Mobile")
                        .IsRequired()
                        .HasMaxLength(20);

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256);

                    b.Property<string>("PasswordHash");

                    b.Property<long?>("PharmacyId");

                    b.Property<string>("PhoneNumber")
                        .HasMaxLength(12);

                    b.Property<bool>("PhoneNumberConfirmed");

                    b.Property<string>("SecurityStamp");

                    b.Property<bool>("TwoFactorEnabled");

                    b.Property<string>("UserName")
                        .HasMaxLength(100);

                    b.Property<string>("UserProfileImage")
                        .HasMaxLength(100);

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.HasIndex("NormalizedEmail")
                        .HasName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.HasIndex("PharmacyId");

                    b.ToTable("AspNetUsers");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.AccessModuleFunctionality", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("FunctionalityId");

                    b.Property<bool>("IsActive");

                    b.Property<int>("ModuleId");

                    b.HasKey("Id");

                    b.HasIndex("FunctionalityId");

                    b.HasIndex("ModuleId");

                    b.ToTable("AccessModuleFunctionality");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.AdvertiseEmailTemplate", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Template")
                        .IsRequired();

                    b.Property<string>("TemplateName")
                        .IsRequired();

                    b.HasKey("Id");

                    b.ToTable("AdvertiseEmailTemplate");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Advertisement", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<DateTime?>("DealDate");

                    b.Property<short>("DealType");

                    b.Property<long>("DistributorId");

                    b.Property<DateTime?>("EndDate");

                    b.Property<bool>("IsActive");

                    b.Property<DateTime?>("LastSentDate");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Notes");

                    b.Property<DateTime?>("PriceIncreaseDate");

                    b.Property<string>("Request");

                    b.Property<DateTime?>("StartDate");

                    b.Property<bool?>("Status");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.ToTable("Advertisements");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.AdvertisementMedicine", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long>("AdvertisementId");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<float>("DealPrice");

                    b.Property<bool>("IsActive");

                    b.Property<long>("MedicineId");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.HasIndex("AdvertisementId");

                    b.HasIndex("MedicineId");

                    b.ToTable("AdvertisementMedicines");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Blog", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long>("AuthorId");

                    b.Property<long>("BlogCategoryId");

                    b.Property<DateTime>("BlogDate");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Descriptions")
                        .IsRequired();

                    b.Property<string>("InternalName")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("RelatedBlogs");

                    b.Property<string>("Title")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("AuthorId");

                    b.HasIndex("BlogCategoryId");

                    b.ToTable("Blog");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.BlogCategory", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("CategoryName")
                        .IsRequired();

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.ToTable("BlogCategory");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.BlogImage", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long>("BlogId");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("ImageName")
                        .IsRequired();

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.HasIndex("BlogId");

                    b.ToTable("BlogImage");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.BlogTag", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long>("BlogId");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("TagName")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("BlogId");

                    b.ToTable("BlogTags");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Cart", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<DateTime>("CartDate");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long>("DistributorId");

                    b.Property<bool>("IsActive");

                    b.Property<long>("MedicineId");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<long>("PharmacyId");

                    b.Property<int>("Quantity");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.HasIndex("MedicineId");

                    b.HasIndex("PharmacyId");

                    b.ToTable("Cart");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.CommissionHistory", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal>("CommissionAmount")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<short>("CommissionStatus");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long>("DistributorId");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.ToTable("CommissionHistory");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.ContactDetails", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(150);

                    b.Property<string>("City")
                        .IsRequired()
                        .HasMaxLength(100);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(256);

                    b.Property<string>("Fax")
                        .HasMaxLength(15);

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("State")
                        .IsRequired()
                        .HasMaxLength(100);

                    b.Property<string>("Telephone")
                        .IsRequired()
                        .HasMaxLength(20);

                    b.Property<string>("ZipCode")
                        .IsRequired()
                        .HasMaxLength(10);

                    b.HasKey("Id");

                    b.ToTable("ContactDetails");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.ContactRequest", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("CompanyName")
                        .HasMaxLength(50);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(256);

                    b.Property<bool>("IsActive");

                    b.Property<string>("Message")
                        .IsRequired();

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(60);

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(20);

                    b.HasKey("Id");

                    b.ToTable("ContactRequest");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Distributor", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(150);

                    b.Property<string>("City")
                        .IsRequired()
                        .HasMaxLength(100);

                    b.Property<string>("CompanyLogo")
                        .HasMaxLength(50);

                    b.Property<string>("CompanyName")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.Property<string>("ContactAddress")
                        .HasMaxLength(100);

                    b.Property<string>("ContactCity");

                    b.Property<string>("ContactEmail")
                        .IsRequired()
                        .HasMaxLength(256);

                    b.Property<string>("ContactMobile")
                        .IsRequired()
                        .HasMaxLength(20);

                    b.Property<string>("ContactName")
                        .IsRequired()
                        .HasMaxLength(100);

                    b.Property<long?>("ContactStateId");

                    b.Property<string>("ContactZipCode");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(256);

                    b.Property<bool>("IsActive");

                    b.Property<string>("Mobile")
                        .IsRequired()
                        .HasMaxLength(20);

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Phone")
                        .HasMaxLength(20);

                    b.Property<long>("StateId");

                    b.Property<long?>("UserId");

                    b.Property<string>("ZipCode")
                        .IsRequired()
                        .HasMaxLength(10);

                    b.HasKey("Id");

                    b.HasIndex("ContactStateId");

                    b.HasIndex("StateId");

                    b.HasIndex("UserId");

                    b.ToTable("Distributor");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorDocumentMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long>("DistributorId");

                    b.Property<bool>("IsActive");

                    b.Property<string>("LicenseCertificate");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("ReturnPolicy");

                    b.Property<string>("Waiver");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId")
                        .IsUnique();

                    b.ToTable("DistributorDocumentMasters");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorMedicine", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long>("DistributorId");

                    b.Property<bool>("IsActive");

                    b.Property<long>("MedicineId");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.HasIndex("MedicineId");

                    b.ToTable("DistributorMedicines");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorOrder", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long?>("DistributorId");

                    b.Property<bool>("IsActive");

                    b.Property<long>("MedicineId");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<long>("OrderChargeId");

                    b.Property<decimal>("Price")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<int>("Quantity");

                    b.Property<decimal>("TotalPrice")
                        .HasColumnType("decimal(17, 2)");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.HasIndex("MedicineId");

                    b.HasIndex("OrderChargeId");

                    b.ToTable("DistributorOrder");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorOrderCharge", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<DateTime?>("CommissionCountDate");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long?>("DistributorId");

                    b.Property<decimal>("GrandTotal")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<long>("OrderId");

                    b.Property<short>("OrderStatus");

                    b.Property<decimal?>("OverNightCharge")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<decimal?>("PaymentAmount")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<DateTime?>("PaymentDate");

                    b.Property<string>("PaymentNote");

                    b.Property<DateTime?>("ShippingDate");

                    b.Property<decimal>("ShippingTotal")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<decimal>("SubTotal")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<string>("TrackingLink");

                    b.Property<string>("TrackingNo");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.HasIndex("OrderId");

                    b.ToTable("DistributorOrderCharge");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorOrderSetting", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long>("DistributorId");

                    b.Property<TimeSpan?>("FridayCutOffTime");

                    b.Property<bool>("IsActive");

                    b.Property<decimal>("MinOrderAmount")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<TimeSpan?>("MondayCutOffTime");

                    b.Property<decimal>("OverNightAmount")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<TimeSpan?>("SaturdayCutOffTime");

                    b.Property<bool>("ServiceDayFriday");

                    b.Property<bool>("ServiceDayMonday");

                    b.Property<bool>("ServiceDaySaturday");

                    b.Property<bool>("ServiceDaySunday");

                    b.Property<bool>("ServiceDayThursday");

                    b.Property<bool>("ServiceDayTuesday");

                    b.Property<bool>("ServiceDayWednesday");

                    b.Property<decimal>("ShippingCharge")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<TimeSpan?>("SundayCutOffTime");

                    b.Property<TimeSpan?>("ThursdayCutOffTime");

                    b.Property<long>("TimeZoneId");

                    b.Property<TimeSpan?>("TuesdayCutOffTime");

                    b.Property<TimeSpan?>("WednesdayCutOffTime");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId")
                        .IsUnique();

                    b.HasIndex("TimeZoneId");

                    b.ToTable("DistributorOrderSetting");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorSubscription", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal>("ChargedMonthly")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long>("DistributorId");

                    b.Property<DateTime?>("EndDate");

                    b.Property<bool>("IsActive");

                    b.Property<bool>("IsExpire");

                    b.Property<bool>("IsPayment");

                    b.Property<bool>("IsUpgraded");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Notes");

                    b.Property<string>("PayPalTransactionId");

                    b.Property<DateTime?>("PaymentDate");

                    b.Property<DateTime>("StartDate");

                    b.Property<decimal>("SubscriptionCharge")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long>("SubscriptionTypeId");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.HasIndex("SubscriptionTypeId");

                    b.ToTable("DistributorSubscription");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorSubscriptionHistory", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal>("ChargedMonthly")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long>("DistributorId");

                    b.Property<long?>("DistributorSubscriptionId");

                    b.Property<DateTime?>("EndDate");

                    b.Property<bool>("IsActive");

                    b.Property<bool>("IsPayment");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Notes");

                    b.Property<string>("PayPalTransactionId");

                    b.Property<DateTime?>("PaymentDate");

                    b.Property<DateTime>("StartDate");

                    b.Property<decimal>("SubscriptionCharge")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long>("SubscriptionTypeId");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.HasIndex("DistributorSubscriptionId");

                    b.HasIndex("SubscriptionTypeId");

                    b.ToTable("DistributorSubscriptionHistory");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DocumentMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("DocumentFile")
                        .IsRequired();

                    b.Property<string>("DocumentName")
                        .IsRequired()
                        .HasMaxLength(100);

                    b.Property<int>("DocumentType");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.ToTable("DocumentMaster");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DosageFormMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("DosageForm")
                        .IsRequired()
                        .HasMaxLength(150);

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.ToTable("DosageFormMaster");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.EmailSubscription", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(255);

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.ToTable("EmailSubscriptions");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.ErrorLog", b =>
                {
                    b.Property<long>("LogId")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<DateTime>("LogDate");

                    b.Property<string>("Message");

                    b.Property<string>("Path");

                    b.Property<string>("Source");

                    b.Property<string>("Stack");

                    b.Property<string>("TargetSite");

                    b.Property<string>("Type");

                    b.HasKey("LogId");

                    b.ToTable("ErrorLog");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.FAQs", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Answer")
                        .IsRequired();

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Question")
                        .IsRequired();

                    b.HasKey("Id");

                    b.ToTable("FaQs");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Functionality", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<bool>("IsActive");

                    b.Property<string>("Name");

                    b.HasKey("Id");

                    b.ToTable("Functionality");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Invoice", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long>("CommissionId");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<short>("InvoiceStatus");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.HasIndex("CommissionId");

                    b.ToTable("Invoice");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.InvoicePayment", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long>("InvoiceId");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("PaidBy");

                    b.Property<decimal>("PaymentAmount")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<short>("PaymentStatus");

                    b.HasKey("Id");

                    b.HasIndex("InvoiceId");

                    b.ToTable("InvoicePayment");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.ManufacturerMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<string>("ManufacturerName")
                        .IsRequired()
                        .HasMaxLength(150);

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.ToTable("ManufacturerMaster");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Measurement", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<short>("MeasurementType");

                    b.Property<string>("MeasurementUnit")
                        .IsRequired();

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.ToTable("Measurement");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.MedicineCategoryMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<string>("MedicineCategory")
                        .IsRequired()
                        .HasMaxLength(150);

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.ToTable("MedicineCategoryMaster");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.MedicineImage", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("ImageName");

                    b.Property<bool>("IsActive");

                    b.Property<long>("MedicineId");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.HasKey("Id");

                    b.HasIndex("MedicineId");

                    b.ToTable("MedicineImages");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.MedicineMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("BrandId")
                        .HasColumnName("Brand_Id");

                    b.Property<long?>("Brand_Id")
                        .HasColumnName("MedicineMaster_Brand_Id");

                    b.Property<long?>("CategoryId");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Description");

                    b.Property<long?>("DistributorId");

                    b.Property<long?>("DosageFormId")
                        .HasColumnName("Dosage_Form_Id");

                    b.Property<long?>("Dosage_Form_Id")
                        .HasColumnName("MedicineMaster_Dosage_Form_Id");

                    b.Property<int?>("DrugDescriptorIdentifier")
                        .HasColumnName("Drug_Descriptor_Identifier");

                    b.Property<string>("DrugName")
                        .HasColumnName("Drug_Name")
                        .HasMaxLength(30);

                    b.Property<string>("Flavour")
                        .HasMaxLength(50);

                    b.Property<string>("GenericProductIdentifier")
                        .HasColumnName("Generic_Product_Identifier")
                        .HasMaxLength(14);

                    b.Property<string>("GenericProductPackagingCode")
                        .HasColumnName("Generic_Product_Packaging_Code")
                        .HasMaxLength(8);

                    b.Property<bool>("IsActive");

                    b.Property<bool>("IsDelete");

                    b.Property<bool>("IsDiscontinue");

                    b.Property<bool>("IsNdc")
                        .HasColumnName("IsNDC");

                    b.Property<long?>("LastChangeDate")
                        .HasColumnName("Last_Change_Date");

                    b.Property<long?>("ManufacturerId")
                        .HasColumnName("Manufacturer_Id");

                    b.Property<long?>("Manufacturer_Id")
                        .HasColumnName("MedicineMaster_Manufacturer_Id");

                    b.Property<string>("MedicineImage");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("NdcUpcHri")
                        .IsRequired()
                        .HasColumnName("NDC_UPC_HRI")
                        .HasMaxLength(11);

                    b.Property<string>("NewNdcUpcHri")
                        .HasColumnName("New_NDC_UPC_HRI")
                        .HasMaxLength(11);

                    b.Property<string>("OldNdcUpcHri")
                        .HasColumnName("Old_NDC_UPC_HRI")
                        .HasMaxLength(11);

                    b.Property<long?>("PackageDescriptionCodeId")
                        .HasColumnName("Package_Description_Code_Id");

                    b.Property<int?>("PackageQuantity")
                        .HasColumnName("Package_Quantity");

                    b.Property<float?>("PackageSize")
                        .HasColumnName("Package_Size");

                    b.Property<long?>("PackageSizeId")
                        .HasColumnName("Package_Size_Id");

                    b.Property<long?>("Package_Description_Code_Id")
                        .HasColumnName("MedicineMaster_Package_Description_Code_Id");

                    b.Property<long?>("Package_Size_Id")
                        .HasColumnName("MedicineMaster_Package_Size_Id");

                    b.Property<string>("Reason")
                        .HasMaxLength(300);

                    b.Property<string>("Strength")
                        .HasMaxLength(15);

                    b.Property<long?>("StrengthId")
                        .HasColumnName("Strength_Id");

                    b.Property<long?>("Strength_Id")
                        .HasColumnName("MedicineMaster_Strength_Id");

                    b.Property<long?>("UnitSizeId")
                        .HasColumnName("Unit_Size_Id");

                    b.Property<long?>("Unit_Size_Id")
                        .HasColumnName("MedicineMaster_Unit_Size_Id");

                    b.HasKey("Id");

                    b.HasIndex("Brand_Id");

                    b.HasIndex("CategoryId");

                    b.HasIndex("DistributorId");

                    b.HasIndex("Dosage_Form_Id");

                    b.HasIndex("Manufacturer_Id");

                    b.HasIndex("Package_Description_Code_Id");

                    b.HasIndex("Package_Size_Id");

                    b.HasIndex("Strength_Id");

                    b.HasIndex("Unit_Size_Id");

                    b.ToTable("MedicineMasters");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.MedicinePriceHistory", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<float?>("AwppackagePrice")
                        .HasColumnName("AWPPackage_Price");

                    b.Property<float?>("AwpunitPrice")
                        .HasColumnName("AWPUnit_Price");

                    b.Property<float?>("AwpunitPriceExtended")
                        .HasColumnName("AWPUnit_Price_Extended");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("InStock");

                    b.Property<bool>("IsContracted");

                    b.Property<bool>("IsShortDated");

                    b.Property<long>("MedicinePriceId");

                    b.Property<long>("ModifiedBy");

                    b.Property<float?>("WacpackagePrice")
                        .HasColumnName("WACPackage_Price");

                    b.Property<float?>("WacunitPrice")
                        .HasColumnName("WACUnit_Price");

                    b.Property<float?>("WacunitPriceExtended")
                        .HasColumnName("WACUnit_Price_Extended");

                    b.HasKey("Id");

                    b.HasIndex("MedicinePriceId");

                    b.ToTable("MedicinePriceHistory");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.MedicinePriceMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<float?>("AwppackagePrice")
                        .HasColumnName("AWPPackage_Price");

                    b.Property<float?>("AwpunitPrice")
                        .HasColumnName("AWPUnit_Price");

                    b.Property<float?>("AwpunitPriceExtended")
                        .HasColumnName("AWPUnit_Price_Extended");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long?>("DistributorId");

                    b.Property<long?>("EffectiveDate")
                        .HasColumnName("Effective_Date");

                    b.Property<string>("GenericProductPackagingCode")
                        .HasColumnName("Generic_Product_Packaging_Code")
                        .HasMaxLength(8);

                    b.Property<string>("GppcPriceCode")
                        .HasColumnName("GPPC_Price_Code")
                        .HasMaxLength(1);

                    b.Property<float?>("GppcUnitPrice")
                        .HasColumnName("GPPC_Unit_Price");

                    b.Property<bool>("InStock");

                    b.Property<bool>("IsActive");

                    b.Property<bool>("IsContracted");

                    b.Property<bool>("IsShortDated");

                    b.Property<long>("MedicineId");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("NdcUpcHri")
                        .IsRequired()
                        .HasColumnName("NDC_UPC_HRI")
                        .HasMaxLength(11);

                    b.Property<float?>("WacpackagePrice")
                        .HasColumnName("WACPackage_Price");

                    b.Property<float?>("WacunitPrice")
                        .HasColumnName("WACUnit_Price");

                    b.Property<float?>("WacunitPriceExtended")
                        .HasColumnName("WACUnit_Price_Extended");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.HasIndex("MedicineId");

                    b.ToTable("MedicinePriceMasters");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.NewDistributorRequest", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(150);

                    b.Property<string>("City")
                        .IsRequired()
                        .HasMaxLength(100);

                    b.Property<string>("CompanyName")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.Property<string>("ContactAddress")
                        .HasMaxLength(100);

                    b.Property<string>("ContactCity");

                    b.Property<string>("ContactEmail")
                        .IsRequired()
                        .HasMaxLength(256);

                    b.Property<string>("ContactMobile")
                        .IsRequired()
                        .HasMaxLength(20);

                    b.Property<string>("ContactName")
                        .IsRequired()
                        .HasMaxLength(100);

                    b.Property<long?>("ContactStateId");

                    b.Property<string>("ContactZipCode");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(256);

                    b.Property<bool>("IsActive");

                    b.Property<string>("Mobile")
                        .IsRequired()
                        .HasMaxLength(20);

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Phone")
                        .HasMaxLength(20);

                    b.Property<long>("StateId");

                    b.Property<string>("ZipCode")
                        .IsRequired()
                        .HasMaxLength(10);

                    b.HasKey("Id");

                    b.HasIndex("ContactStateId");

                    b.HasIndex("StateId");

                    b.ToTable("NewDistributorRequest");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Order", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("BillingAddress");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<DateTime?>("DeliveryDate");

                    b.Property<bool>("DeliveryStatus");

                    b.Property<bool>("IsActive");

                    b.Property<bool>("IsOverNight");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<DateTime>("OrderDate");

                    b.Property<decimal>("OrderGrandTotal")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<decimal>("OrderShippingTotal")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<decimal>("OrderSubTotal")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long?>("PharmacyId");

                    b.Property<string>("ShippingAddress");

                    b.Property<string>("UniqueOrder");

                    b.HasKey("Id");

                    b.HasIndex("CreatedBy");

                    b.HasIndex("PharmacyId");

                    b.ToTable("Order");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Pharmacy", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<DateTime>("DeaExpires");

                    b.Property<string>("DeaFile")
                        .IsRequired();

                    b.Property<string>("DeaNumber")
                        .IsRequired()
                        .HasMaxLength(9);

                    b.Property<bool>("IsActive");

                    b.Property<DateTime>("LicenseExpires");

                    b.Property<string>("LicenseFile")
                        .IsRequired();

                    b.Property<string>("LicenseNumber")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("NpiNumber")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.Property<string>("PharmacyName")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.Property<long>("PharmacySystemId");

                    b.Property<long>("PharmacyTypeId");

                    b.Property<string>("ReferCode");

                    b.Property<int>("Status");

                    b.Property<long>("UserId");

                    b.HasKey("Id");

                    b.HasIndex("PharmacySystemId");

                    b.HasIndex("PharmacyTypeId");

                    b.HasIndex("UserId");

                    b.ToTable("Pharmacy");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.PharmacyBillingAddress", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Address1")
                        .IsRequired();

                    b.Property<string>("Address2");

                    b.Property<string>("City")
                        .IsRequired();

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<bool>("IsDefault");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<long>("PharmacyId");

                    b.Property<long>("StateId");

                    b.Property<string>("ZipCode")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("PharmacyId");

                    b.HasIndex("StateId");

                    b.ToTable("PharmacyBillingAddress");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.PharmacyShippingAddress", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Address1")
                        .IsRequired();

                    b.Property<string>("Address2");

                    b.Property<string>("City")
                        .IsRequired();

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<bool>("IsDefault");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<long>("PharmacyId");

                    b.Property<long>("StateId");

                    b.Property<string>("ZipCode")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("PharmacyId");

                    b.HasIndex("StateId");

                    b.ToTable("PharmacyShippingAddress");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.PharmacySystemMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("PharmacySystemName")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.HasKey("Id");

                    b.ToTable("PharmacySystemMaster");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.PharmacyTypeMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("PharmacyTypeName")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.HasKey("Id");

                    b.ToTable("PharmacyTypeMaster");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RedeemRequest", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<short>("DeliveryStatus");

                    b.Property<bool>("IsActive");

                    b.Property<bool?>("IsApprove");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<long>("PharmacyId");

                    b.Property<long>("RewardProductId");

                    b.HasKey("Id");

                    b.HasIndex("PharmacyId");

                    b.HasIndex("RewardProductId");

                    b.ToTable("RedeemRequest");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RewardEarn", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<long?>("OrderId");

                    b.Property<long?>("PharmacyId");

                    b.Property<string>("RewardBy");

                    b.Property<decimal>("RewardMoney")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long>("RewardTypeId");

                    b.HasKey("Id");

                    b.HasIndex("OrderId");

                    b.HasIndex("PharmacyId");

                    b.HasIndex("RewardTypeId");

                    b.ToTable("RewardEarn");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RewardMoneyMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(150);

                    b.Property<bool>("IsActive");

                    b.Property<decimal>("MaxRange")
                        .HasColumnType("decimal(10, 2)");

                    b.Property<decimal>("MinRange")
                        .HasColumnType("decimal(10, 2)");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<decimal>("Referral")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long>("RewardTypeId");

                    b.HasKey("Id");

                    b.HasIndex("RewardTypeId");

                    b.ToTable("RewardMoneyMaster");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RewardMonthDays", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("NoOfDays");

                    b.HasKey("Id");

                    b.ToTable("RewardMonthDays");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RewardProduct", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Description");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("ProductImage")
                        .IsRequired()
                        .HasColumnType("varchar(100)");

                    b.Property<string>("ProductName")
                        .IsRequired()
                        .HasColumnType("varchar(100)");

                    b.Property<decimal>("Redeem")
                        .HasColumnType("decimal(17, 2)");

                    b.HasKey("Id");

                    b.ToTable("RewardProduct");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RewardTypeMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Type")
                        .HasMaxLength(50);

                    b.HasKey("Id");

                    b.ToTable("RewardTypeMaster");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RolesModuleAccess", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("ModuleId");

                    b.Property<long>("RoleId");

                    b.HasKey("Id");

                    b.HasIndex("ModuleId");

                    b.HasIndex("RoleId");

                    b.ToTable("RolesModuleAccess");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RolesModuleFunctionalityAccess", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("FunctionalityId");

                    b.Property<int>("RolesModuleId");

                    b.HasKey("Id");

                    b.HasIndex("FunctionalityId");

                    b.HasIndex("RolesModuleId");

                    b.ToTable("RolesModuleFunctionalityAccess");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RxExtDbMedicine", b =>
                {
                    b.Property<long>("Row")
                        .HasColumnName("Row#");

                    b.Property<string>("BrandNameCode")
                        .HasColumnName("Brand_Name_Code")
                        .HasMaxLength(1);

                    b.Property<string>("DosageForm")
                        .HasColumnName("Dosage_Form")
                        .HasMaxLength(4);

                    b.Property<int?>("DrugDescriptorIdentifier")
                        .HasColumnName("Drug_Descriptor_Identifier");

                    b.Property<string>("DrugName")
                        .HasColumnName("Drug_Name")
                        .HasMaxLength(30);

                    b.Property<string>("GenericProductIdentifier")
                        .HasColumnName("Generic_Product_Identifier")
                        .HasMaxLength(14);

                    b.Property<string>("GenericProductPackagingCode")
                        .HasColumnName("Generic_Product_Packaging_Code")
                        .HasMaxLength(8);

                    b.Property<string>("GppcPriceCode")
                        .HasColumnName("GPPC_Price_Code")
                        .HasMaxLength(1);

                    b.Property<float?>("GppcUnitPrice")
                        .HasColumnName("GPPC_Unit_Price");

                    b.Property<bool>("IsDuplicate");

                    b.Property<long?>("LastChangeDate")
                        .HasColumnName("Last_Change_Date");

                    b.Property<string>("Manufacturer")
                        .HasColumnName("Manufacturers_Labeler_Name")
                        .HasMaxLength(50);

                    b.Property<string>("MedicineImage")
                        .HasColumnName("Image_Filename")
                        .HasMaxLength(20);

                    b.Property<string>("NdcUpcHri")
                        .IsRequired()
                        .HasColumnName("NDC_UPC_HRI")
                        .HasMaxLength(11);

                    b.Property<string>("NewNdcUpcHri")
                        .HasColumnName("New_NDC_UPC_HRI")
                        .HasMaxLength(11);

                    b.Property<string>("OldNdcUpcHri")
                        .HasColumnName("Old_NDC_UPC_HRI")
                        .HasMaxLength(11);

                    b.Property<string>("PackageDescriptionCode")
                        .HasColumnName("Package_Description_Code")
                        .HasMaxLength(2);

                    b.Property<int?>("PackageQuantity")
                        .HasColumnName("Package_Quantity");

                    b.Property<float?>("PackageSize")
                        .HasColumnName("Package_Size");

                    b.Property<string>("PackageSizeUnitOfMeasure")
                        .HasColumnName("Package_Size_Unit_of_Measure")
                        .HasMaxLength(2);

                    b.Property<string>("Strength")
                        .HasMaxLength(15);

                    b.Property<string>("StrengthUnitOfMeasure")
                        .HasColumnName("Strength_Unit_of_Measure")
                        .HasMaxLength(10);

                    b.Property<string>("UnitDoseOfUsePackage")
                        .HasColumnName("Unit_DoseUnit_of_Use_Package")
                        .HasMaxLength(1);

                    b.HasKey("Row");

                    b.ToTable("RxExtDbMedicines");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.State", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100);

                    b.HasKey("Id");

                    b.ToTable("State");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.SubscriptionType", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal>("ChargedMonthly")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Description");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<decimal>("SubscriptionCharge")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<string>("SubscriptionTypeName")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.HasKey("Id");

                    b.ToTable("SubscriptionType");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.SubscriptionTypeHistory", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal>("ChargedMonthly")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<decimal>("SubscriptionCharge")
                        .HasColumnType("decimal(17, 2)");

                    b.Property<long>("SubscriptionTypeId");

                    b.HasKey("Id");

                    b.HasIndex("SubscriptionTypeId");

                    b.ToTable("SubscriptionTypeHistory");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.SystemModule", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Action");

                    b.Property<string>("Controller");

                    b.Property<bool>("IsField");

                    b.Property<string>("MenuDisplayText");

                    b.Property<string>("ModuleName");

                    b.Property<int?>("ParentsId");

                    b.Property<int>("UserGroupId");

                    b.HasKey("Id");

                    b.HasIndex("ParentsId");

                    b.ToTable("SystemModule");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.TermsAndCondition", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("TermsCondition")
                        .IsRequired();

                    b.HasKey("Id");

                    b.ToTable("TermsAndCondition");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Testimonials", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Feedback")
                        .IsRequired();

                    b.Property<string>("Image");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Name")
                        .IsRequired();

                    b.HasKey("Id");

                    b.ToTable("Testimonials");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.TimeZoneMaster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("DaylightName")
                        .IsRequired()
                        .HasMaxLength(250);

                    b.Property<string>("DisplayName")
                        .IsRequired()
                        .HasMaxLength(250);

                    b.Property<bool>("IsActive");

                    b.Property<bool>("IsDaylight");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("StandardName")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.Property<string>("TimeZoneId")
                        .IsRequired()
                        .HasMaxLength(250);

                    b.Property<string>("UtcDaylightOffset")
                        .IsRequired()
                        .HasMaxLength(10);

                    b.Property<string>("UtcStandardOffset")
                        .IsRequired()
                        .HasMaxLength(10);

                    b.HasKey("Id");

                    b.ToTable("TimeZoneMaster");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.UploadedMedicine", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Brand")
                        .HasMaxLength(4);

                    b.Property<string>("Category");

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Description");

                    b.Property<long>("DistributorId");

                    b.Property<string>("DosageForm")
                        .HasMaxLength(4);

                    b.Property<string>("Flavour")
                        .HasMaxLength(50);

                    b.Property<bool>("IsActive");

                    b.Property<bool?>("IsApproved");

                    b.Property<string>("Manufacturer");

                    b.Property<string>("MedicineImage");

                    b.Property<string>("MedicineName")
                        .HasMaxLength(30);

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<string>("Ndc")
                        .IsRequired()
                        .HasMaxLength(11);

                    b.Property<float?>("PackageSize")
                        .HasColumnName("PackageSize");

                    b.Property<string>("Reason")
                        .HasMaxLength(300);

                    b.Property<string>("Strength")
                        .HasMaxLength(15);

                    b.Property<string>("Unit")
                        .HasColumnName("Unit")
                        .HasMaxLength(1);

                    b.Property<string>("Upc")
                        .HasMaxLength(11);

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.ToTable("UploadedMedicines");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.UserAddress", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Address1")
                        .HasMaxLength(256);

                    b.Property<string>("Address2")
                        .HasMaxLength(256);

                    b.Property<string>("City")
                        .HasMaxLength(50);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<bool>("IsActive");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<long?>("StateId");

                    b.Property<string>("ZipCode")
                        .HasMaxLength(10);

                    b.HasKey("Id");

                    b.HasIndex("CreatedBy");

                    b.HasIndex("StateId");

                    b.ToTable("UserAddresses");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.WatchList", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long?>("CreatedBy");

                    b.Property<DateTime>("CreatedDate");

                    b.Property<long>("DistributorId");

                    b.Property<bool>("IsActive");

                    b.Property<float>("MatchPrice");

                    b.Property<DateTime>("MedicineDate");

                    b.Property<long>("MedicineId");

                    b.Property<long?>("ModifiedBy");

                    b.Property<DateTime?>("ModifiedDate");

                    b.Property<long>("PharmacyId");

                    b.Property<int>("Quantity");

                    b.HasKey("Id");

                    b.HasIndex("DistributorId");

                    b.HasIndex("MedicineId");

                    b.HasIndex("PharmacyId");

                    b.ToTable("WatchList");
                });

            modelBuilder.Entity("RxFair.Data.DbContext.Role", b =>
                {
                    b.HasBaseType("Microsoft.AspNetCore.Identity.IdentityRole<long>");

                    b.Property<string>("DisplayRoleName");

                    b.HasDiscriminator().HasValue("Role");
                });

            modelBuilder.Entity("RxFair.Data.DbContext.UserClaim", b =>
                {
                    b.HasBaseType("Microsoft.AspNetCore.Identity.IdentityUserClaim<long>");

                    b.HasDiscriminator().HasValue("UserClaim");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<long>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole<long>")
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<long>", b =>
                {
                    b.HasOne("RxFair.Data.DbContext.ApplicationUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<long>", b =>
                {
                    b.HasOne("RxFair.Data.DbContext.ApplicationUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<long>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole<long>")
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbContext.ApplicationUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<long>", b =>
                {
                    b.HasOne("RxFair.Data.DbContext.ApplicationUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbContext.ApplicationUser", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithMany("DistributorUsers")
                        .HasForeignKey("DistributorId");

                    b.HasOne("RxFair.Data.DbModel.Pharmacy", "Pharmacy")
                        .WithMany("PharmacyUsers")
                        .HasForeignKey("PharmacyId");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.AccessModuleFunctionality", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Functionality", "Functionality")
                        .WithMany("AccessModuleFunctionalities")
                        .HasForeignKey("FunctionalityId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.SystemModule", "SystemModule")
                        .WithMany("AccessModuleFunctionalities")
                        .HasForeignKey("ModuleId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Advertisement", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithMany("Advertisements")
                        .HasForeignKey("DistributorId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.AdvertisementMedicine", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Advertisement", "Advertisement")
                        .WithMany("AdvertisementMedicines")
                        .HasForeignKey("AdvertisementId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.MedicineMaster", "MedicineMaster")
                        .WithMany("AdvertisementMedicines")
                        .HasForeignKey("MedicineId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Blog", b =>
                {
                    b.HasOne("RxFair.Data.DbContext.ApplicationUser", "Author")
                        .WithMany("AuthorBlogs")
                        .HasForeignKey("AuthorId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.BlogCategory", "BlogCategory")
                        .WithMany("Blogs")
                        .HasForeignKey("BlogCategoryId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.BlogImage", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Blog", "Blog")
                        .WithMany("BlogImages")
                        .HasForeignKey("BlogId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.BlogTag", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Blog", "Blog")
                        .WithMany("BlogTags")
                        .HasForeignKey("BlogId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Cart", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithMany("Carts")
                        .HasForeignKey("DistributorId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.MedicineMaster", "MedicineMaster")
                        .WithMany("Carts")
                        .HasForeignKey("MedicineId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.Pharmacy", "Pharmacy")
                        .WithMany("Carts")
                        .HasForeignKey("PharmacyId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.CommissionHistory", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithMany("CommissionHistories")
                        .HasForeignKey("DistributorId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Distributor", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.State", "ContactState")
                        .WithMany("ContactState")
                        .HasForeignKey("ContactStateId");

                    b.HasOne("RxFair.Data.DbModel.State", "State")
                        .WithMany("Distributors")
                        .HasForeignKey("StateId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbContext.ApplicationUser", "DistributorAdminUser")
                        .WithMany("DistributorOwner")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorDocumentMaster", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithOne("DistributorDocumentMaster")
                        .HasForeignKey("RxFair.Data.DbModel.DistributorDocumentMaster", "DistributorId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorMedicine", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithMany("DistributorMedicines")
                        .HasForeignKey("DistributorId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.MedicineMaster", "MedicineMaster")
                        .WithMany("DistributorMedicines")
                        .HasForeignKey("MedicineId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorOrder", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithMany("DistributorOrders")
                        .HasForeignKey("DistributorId");

                    b.HasOne("RxFair.Data.DbModel.MedicineMaster", "MedicineMaster")
                        .WithMany("DistributorOrders")
                        .HasForeignKey("MedicineId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.DistributorOrderCharge", "OrderCharge")
                        .WithMany("DistributorOrders")
                        .HasForeignKey("OrderChargeId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorOrderCharge", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithMany("DistributorOrderCharges")
                        .HasForeignKey("DistributorId");

                    b.HasOne("RxFair.Data.DbModel.Order", "Order")
                        .WithMany("OrderCharges")
                        .HasForeignKey("OrderId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorOrderSetting", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithOne("DistributorOrderSettings")
                        .HasForeignKey("RxFair.Data.DbModel.DistributorOrderSetting", "DistributorId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.TimeZoneMaster", "TimeZoneMaster")
                        .WithMany("DistributorOrderSettings")
                        .HasForeignKey("TimeZoneId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorSubscription", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithMany("DistributorSubscriptions")
                        .HasForeignKey("DistributorId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.SubscriptionType", "SubscriptionType")
                        .WithMany("DistributorSubscriptions")
                        .HasForeignKey("SubscriptionTypeId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.DistributorSubscriptionHistory", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithMany()
                        .HasForeignKey("DistributorId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.DistributorSubscription")
                        .WithMany("DistributorSubscriptionHistories")
                        .HasForeignKey("DistributorSubscriptionId");

                    b.HasOne("RxFair.Data.DbModel.SubscriptionType", "SubscriptionType")
                        .WithMany()
                        .HasForeignKey("SubscriptionTypeId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Invoice", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.CommissionHistory", "CommissionHistory")
                        .WithMany("Invoices")
                        .HasForeignKey("CommissionId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.InvoicePayment", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Invoice", "Invoice")
                        .WithMany("InvoicePayments")
                        .HasForeignKey("InvoiceId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.MedicineImage", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.MedicineMaster", "MedicineMaster")
                        .WithMany("MedicineImages")
                        .HasForeignKey("MedicineId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.MedicineMaster", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Measurement", "BrandMeasurement")
                        .WithMany("BrandMeasurement")
                        .HasForeignKey("Brand_Id");

                    b.HasOne("RxFair.Data.DbModel.MedicineCategoryMaster", "CategoryMaster")
                        .WithMany("MedicineMasters")
                        .HasForeignKey("CategoryId");

                    b.HasOne("RxFair.Data.DbModel.Distributor", "DistributorMedicine")
                        .WithMany("MedicineMasters")
                        .HasForeignKey("DistributorId");

                    b.HasOne("RxFair.Data.DbModel.DosageFormMaster", "DosageFormMaster")
                        .WithMany("MedicineMasters")
                        .HasForeignKey("Dosage_Form_Id");

                    b.HasOne("RxFair.Data.DbModel.ManufacturerMaster", "ManufacturerMaster")
                        .WithMany("MedicineMasters")
                        .HasForeignKey("Manufacturer_Id");

                    b.HasOne("RxFair.Data.DbModel.Measurement", "PackageDescriptionCodeMeasurement")
                        .WithMany("PackageDescriptionCodeMeasurement")
                        .HasForeignKey("Package_Description_Code_Id");

                    b.HasOne("RxFair.Data.DbModel.Measurement", "PackageMeasurement")
                        .WithMany("PackageMeasurement")
                        .HasForeignKey("Package_Size_Id");

                    b.HasOne("RxFair.Data.DbModel.Measurement", "StrengthMeasurement")
                        .WithMany("StrengthMeasurement")
                        .HasForeignKey("Strength_Id");

                    b.HasOne("RxFair.Data.DbModel.Measurement", "UnitMeasurement")
                        .WithMany("UnitMeasurement")
                        .HasForeignKey("Unit_Size_Id");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.MedicinePriceHistory", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.MedicinePriceMaster", "MedicinePriceMaster")
                        .WithMany()
                        .HasForeignKey("MedicinePriceId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.MedicinePriceMaster", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "DistributorMedicine")
                        .WithMany("MedicinePriceMasters")
                        .HasForeignKey("DistributorId");

                    b.HasOne("RxFair.Data.DbModel.MedicineMaster", "MedicineMaster")
                        .WithMany("MedicinePriceMasters")
                        .HasForeignKey("MedicineId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.NewDistributorRequest", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.State", "ContactState")
                        .WithMany()
                        .HasForeignKey("ContactStateId");

                    b.HasOne("RxFair.Data.DbModel.State", "State")
                        .WithMany()
                        .HasForeignKey("StateId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Order", b =>
                {
                    b.HasOne("RxFair.Data.DbContext.ApplicationUser", "OrderCreatedBy")
                        .WithMany()
                        .HasForeignKey("CreatedBy");

                    b.HasOne("RxFair.Data.DbModel.Pharmacy", "Pharmacy")
                        .WithMany("Orders")
                        .HasForeignKey("PharmacyId");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.Pharmacy", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.PharmacySystemMaster", "PharmacySystemMaster")
                        .WithMany("Pharmacies")
                        .HasForeignKey("PharmacySystemId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.PharmacyTypeMaster", "PharmacyTypeMaster")
                        .WithMany("Pharmacies")
                        .HasForeignKey("PharmacyTypeId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbContext.ApplicationUser", "PharmacyAdminUser")
                        .WithMany("PharmacyOwner")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.PharmacyBillingAddress", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Pharmacy", "Pharmacy")
                        .WithMany("BillingAddresses")
                        .HasForeignKey("PharmacyId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.State", "State")
                        .WithMany("PharmacyBillingAddresses")
                        .HasForeignKey("StateId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.PharmacyShippingAddress", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Pharmacy", "Pharmacy")
                        .WithMany("ShippingAddresses")
                        .HasForeignKey("PharmacyId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.State", "State")
                        .WithMany("PharmacyShippingAddresses")
                        .HasForeignKey("StateId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RedeemRequest", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Pharmacy", "Pharmacy")
                        .WithMany("RedeemRequests")
                        .HasForeignKey("PharmacyId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.RewardProduct", "RewardProduct")
                        .WithMany("RedeemRequests")
                        .HasForeignKey("RewardProductId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RewardEarn", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Order", "Order")
                        .WithMany("RewardEarns")
                        .HasForeignKey("OrderId");

                    b.HasOne("RxFair.Data.DbModel.Pharmacy", "Pharmacy")
                        .WithMany("RewardEarns")
                        .HasForeignKey("PharmacyId");

                    b.HasOne("RxFair.Data.DbModel.RewardTypeMaster", "RewardTypeMaster")
                        .WithMany("RewardEarns")
                        .HasForeignKey("RewardTypeId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RewardMoneyMaster", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.RewardTypeMaster", "RewardTypeMaster")
                        .WithMany("RewardMoneyMasters")
                        .HasForeignKey("RewardTypeId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RolesModuleAccess", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.SystemModule", "SystemModule")
                        .WithMany("RolesModuleAccess")
                        .HasForeignKey("ModuleId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbContext.Role", "Roles")
                        .WithMany("RolesModuleAccesses")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.RolesModuleFunctionalityAccess", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Functionality", "Functionality")
                        .WithMany("RolesModuleFunctionalityAccesses")
                        .HasForeignKey("FunctionalityId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.RolesModuleAccess", "RolesModuleAccess")
                        .WithMany("RolesModuleFunctionalityAccesses")
                        .HasForeignKey("RolesModuleId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.SubscriptionTypeHistory", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.SubscriptionType", "SubscriptionType")
                        .WithMany("SubscriptionTypeHistories")
                        .HasForeignKey("SubscriptionTypeId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.SystemModule", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.SystemModule", "Parent")
                        .WithMany("Children")
                        .HasForeignKey("ParentsId");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.UploadedMedicine", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "DistributorMedicine")
                        .WithMany("UploadedMedicines")
                        .HasForeignKey("DistributorId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("RxFair.Data.DbModel.UserAddress", b =>
                {
                    b.HasOne("RxFair.Data.DbContext.ApplicationUser", "User")
                        .WithMany("UserAddress")
                        .HasForeignKey("CreatedBy");

                    b.HasOne("RxFair.Data.DbModel.State", "State")
                        .WithMany()
                        .HasForeignKey("StateId");
                });

            modelBuilder.Entity("RxFair.Data.DbModel.WatchList", b =>
                {
                    b.HasOne("RxFair.Data.DbModel.Distributor", "Distributor")
                        .WithMany("WatchLists")
                        .HasForeignKey("DistributorId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.MedicineMaster", "MedicineMaster")
                        .WithMany("WatchLists")
                        .HasForeignKey("MedicineId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("RxFair.Data.DbModel.Pharmacy", "Pharmacy")
                        .WithMany("WatchLists")
                        .HasForeignKey("PharmacyId")
                        .OnDelete(DeleteBehavior.Cascade);
                });
#pragma warning restore 612, 618
        }
    }
}
